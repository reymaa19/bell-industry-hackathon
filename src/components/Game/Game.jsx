...
function Game() {
  ...
  return (
        <section className='game'>
            ...
            <p>BlueBells: {score}</p>
            {isWinner && <h2 className='game__winner'>Congrats! You've earned 500 BlueBells!</h2>}
            <section className='game__container'>
                {
                    cards.map(card => (
                        <SingleCard  key={card.id} data = {card} selection = {selection} 
                        flipped = { card === selectionOne || card === selectionTwo || card.matched === true}
                        disabled={disabled}/>
                    ))
                }
